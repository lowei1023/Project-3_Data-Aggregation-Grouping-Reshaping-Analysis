...
class DataRestructure:
    ...
    def calculate_average_generation(self):
        ...
        average_generation = self.df.groupby('Year').agg({
            '1_coal': 'mean',
            '2_petroleum': 'mean',
            '3_Natural_Gas': 'mean',
            '11_solar': 'mean',
    ...
    def reshape_data(self):
        ...
        pivot_data = self.df.pivot_table(index='Year', columns='Month', values=['1_coal', '2_petroleum', '3_Natural_Gas'])
        print(pivot_data)
    ...
    def calculate_mean_by_year_sum_by_month(self):
        ...
        mean_by_year = self.df.groupby('Year')[['1_coal', '2_petroleum', '3_Natural_Gas']].mean()
        sum_by_month = self.df.groupby('Month')[['1_coal', '2_petroleum', '3_Natural_Gas']].sum()
        print("Mean by Year:")
        print(mean_by_year)
        print("\nSum by Month:")
        print(sum_by_month)
    ...
    def reshape_stack_unstack(self):
        ...
        stacked = self.df.set_index(['Year', 'Month'])[['1_coal', '2_petroleum']].stack()
